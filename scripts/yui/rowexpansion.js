        ...
        YAHOO.widget.DataTable, 
        {
			...
			**/
			toggleSingleRowExpansion : function( record_id ) {
				...
				// row_data is null if row is expanded and not last row, and last row is selected to be expanded
				if ( row_data != null ) {
					var state = this._getRecordState( record_id );
					...
					if ( state && state.expanded ){
						...
						// This ensures only one row is expanded at a time
			...
			**/
             ...
             * @method expandRow
             * @param {Mixed} record_id Record / Row / or Index id
             * @param {Boolean} restore will restore an existing state for a
            ...
            expandRow : function( record_id, restore ) {
            	...
            	if( !state.expanded || restore ){
                    ...
                    if ( newRow.innerHTML.length ) {
                        ...
                        if ( !restore ){
                            this.a_rowExpansions.push( Dom.get( record_id ) );
                        }
